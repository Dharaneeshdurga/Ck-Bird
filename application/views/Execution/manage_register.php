...
<html>
    <head>
        ...
        <title><?php echo $this->session->userdata('client_name'); ?> | Management Team</title>
                                                ...
                                                <table id="datatable" class="table table-striped table-bordered">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th>Status by R&D head</th>
                                                            <th>Status by Division head</th>
                                                 ...
                                                 <?php  if (isset($team_result) && !empty($team_result)) { 
                                                         ...
                                                         foreach($team_result as $val) { 
                                                           ...
                                                           <td class="<?php if($val->status_change==1) { echo "danger"; } ?>"><?php echo$req_date;?></td>
                                                        
                                                             <form name="manage_register" id="manage_register" action="<?php echo base_url('index.php/Execution/update_doc'); ?>" enctype="multipart/form-data"  data-parsley-validate method="post" autocomplete="off">
                                                              <?php if ($val->date_doc == ""){ ?>
                                                                <td class="<?php if($val->status_change==1) { echo "danger"; } ?>"> <input type="date" class="form-control" name="doc" id="doc" required/>
                                                               <?php } ?>
                                                               <?php if ($val->date_doc !== ""){ ?>
                                                                <td class="<?php if($val->status_change==1) { echo "danger"; } ?>"> <input type="text" class="form-control" name="doc1" id="doc1" value="<?php echo $val->date_doc;?>" readonly/>
                                                               <?php } ?>
                                                            ...
                                                            <?php if ($val->date_doc == ""){ ?>
                                                                <td class="<?php if($val->status_change==1) { echo "danger"; } ?>"> <button type=submit class="btn btn-purple waves-effect waves-light">Submit</button></td>
                                                          <?php } ?>
                                                        
                                                        </form>
														<td class="<?php if($val->status_change==1) { echo "danger";}?>"><?php echo $val->reason;?></td>
                                                        <?php if ($val->date_doc !== ""){ ?>
                                                            <td class="<?php if($val->status_change==1) { echo "danger"; } ?>"> <button onclick="change_status('<?php echo $val->auto_id;?>');" id="up_status" class="btn btn-success waves-effect waves-light">Change Status</button>
                                                       <div id="cs_status" style="display:none">Closed</div>
                                                        </td>
                                                            <?php } ?>
                                                            <td class="<?php if($val->status_change==1) { echo "danger"; } ?>"> <?php echo $val->status_rd;?></td>
                                                            <td class="<?php if($val->status_change==1) { echo "danger"; } ?>"> <?php echo $val->status_division;?></td>
                                                        </tr>
 ...
 <script src="<?php echo asset_url();?>notifications/notify.min.js"></script>
        ...
        <script type="text/javascript">
        $(document).ready(function () {
				var  page="manage_register";
				
				if(page=="manage_register"){
					$(".excecution_m").click();
