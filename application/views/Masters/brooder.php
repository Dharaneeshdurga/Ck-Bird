...
<html>
    <head>
        ...
        <title><?php echo $this->session->userdata('client_name'); ?> | Add Brooder Details</title>
                                    ...
                                    <div class="modal-dialog"> 
                                        <div class="modal-content"> 
                                            <div class="modal-header"> 
                                                ...
                                                <h4 class="modal-title">ADD BROODER</h4> 
                                            </div> 
                                            <form name="brooder_form" id="brooder_form" class="form-horizontal form-bordered" action="<?php echo base_url('index.php/Masters/add_brooder'); ?>" enctype="multipart/form-data"  data-parsley-validate method="post" autocomplete="off">
                                            <div class="modal-body"> 
                                                <div class="row"> 
                                                    <div class="col-md-6"> 
                                                        <div class="form-group"> 
                                                            <label for="Brooder Name" class="control-label">Brooder Name</label>
                                                             <!-- <input type="hidden" name="auto_id" id="auto_id" class="form-control" value="<?php// $last_id=$last_id['id']+1; echo "B00".$last_id; ?>">  -->
                                                            <input type="hidden" name="created_by" id="created_by" class="form-control" value="<?php echo $this->session->userdata('user_id') ?>"> 
                                                            <input type="text" name="brooder_name" id="brooder_name" class="form-control" required>
                                                        </div> 
                                                        <div class="form-group">
                                                        <label for="Species Name" class="control-label">Target no of Feeds</label>
                                                        <input type="text" name="target_feed" id="target_feed" class="form-control" required>
                                                         </div>
                                                    </div> 
                                            ...
                                            <div class="modal-footer"> 
                                                ...
                                                <button type="submit" class="btn btn-info waves-effect waves-light">Add Brooder</button> 
                                            </div> 
                                            </form>
                                        </div> 
                                    </div>
                            ...
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h3 class="panel-title">Brooders List</h3>
                                    ...
                                    <button class="btn btn-primary waves-effect waves-light" data-toggle="modal" data-target="#con-close-modal">
                                     ...
                                     <!--  --></div>
                                     <div class="col-md-2">
                                         <a href="<?php echo base_url('index.php/Download/brooder_Download'); ?>" class="btn btn-info waves-effect waves-light" ><i class="fa fa-pencil-square-o"></i>Download Sample</a>
                                    </div>
                                    <div class="col-md-8">
                                        <form id="raw_material" action="<?php echo base_url("index.php/Download/brooder_upload") ?>" method="post" enctype="multipart/form-data">
                                            <div class="row">
                                              <div class="col-md-3">  
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-sm-12 col-xs-12">
                                                <table id="datatable" class="table table-striped table-bordered">
                                                    ...
                                                    <tbody>
                                                        <?php $count =0; foreach($brooder as $k => $val ){
                                                            $branch_id = $this->session->userdata('branch_id');
                                                            if($branch_id == $val['branch_id']){
                                                                $count++;?>
                                                            <tr>
                                                                <td class="<?php if($val['status']==0) { echo "danger"; } ?>"><?php echo $count; ?></td>
                                                                <td class="<?php if($val['status']==0) { echo "danger"; } ?>"><?php echo $val['brooder_name']; ?></td>
                                                                <td class="<?php if($val['status']==0) { echo "danger"; } ?>"><?php echo $val['target_feed']; ?></td>
                                                                <td class="<?php if($val['status']==0) { echo "danger"; } ?> text-center"> 
                                                                                             
                                                                <a href="<?php echo base_url('index.php/Masters/edit_brooder/'.$val['id']); ?>" class="btn btn-primary btn-xs" ><i class="fa fa-pencil-square-o"></i></a> &nbsp;&nbsp;
                                                    ...
                                                    <a  onclick="get_delete_pop('<?php echo $val['id'];?>','ckb_brooder');" href="#" class="btn btn-danger btn-xs "><i class="fa fa-close"></i> </a>
        ...
        $(document).ready(function () {
			...
			$(document).ready(function() {
			    $(document).on('click','.inactive_doc',function(){
					var resp = confirm("Do you want to Inactive this Brooder?");
					if (resp == true) {
						var inactive_id = $(this).attr('inactive_id');
						//alert(inactive_id);
						$.ajax({
							type : "POST", 
							url : "<?php echo base_url(); ?>index.php/Masters/inactive_brooder",
			...
			$(document).ready(function() {
			    $(document).on('click','.active_doc',function(){
					var resp = confirm("Do you want to active this brooder?");
					if (resp == true) {
						var active_id = $(this).attr('active_id');
						//alert(active_id);
						$.ajax({
							type : "POST", 
							url : "<?php echo base_url(); ?>index.php/Masters/active_brooder",
  ...
  swal({   
      ...
      text: "You will not be able to recover this BROODER and all the data related this brooder also affected!!!",   
      type: "warning",   
