...
<html>
    <head>
        ...
        <title><?php echo $this->session->userdata('client_name'); ?> | Add Employee Details</title>
                                ...
                                <div id="con-close-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
                                    <div class="modal-dialog"> 
                                        <div class="modal-content"> 
                                            ...
                                            <form name="employe_form" id="employe_form" class="form-horizontal form-bordered" action="<?php echo base_url('index.php/Masters/add_employe'); ?>" enctype="multipart/form-data"  data-parsley-validate method="post" autocomplete="off">
                                                ...
                                                <div class="row"> 
                                                    <div class="col-md-6"> 
                                                        <div class="form-group"> 
                                                            ...
                                                            <input type="text" name="employe_name" id="employe_name" class="form-control" required>
                                                        </div> 
                                                    
                                                        <div class="form-group"> 
                                                            <label for="Employee ID" class="control-label">Employee ID</label>
                                                            <input type="text" name="employe_id" id="employe_id" class="form-control" required>
                                                        </div> 
                                                   
                                                     <div class="form-group">
                                                             <label class="control-label">Select Roles</label>
                                                                <select name="emp_roles" id="emp_roles" class="form-control" required>
                                                                <option value="">Select Roles</option>
                                                                </select>
                                                         </div>
                                                         <div class="form-group">
                                                             <label class="control-label">Select Branch</label>
                                                                <select name="emp_branch" id="emp_branch" class="form-control" required>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-sm-12 col-xs-12">
                                                <table id="datatable" class="table table-striped table-bordered">
                                                    ...
                                                    <tbody>
                                                    <?php 
                                                   // print_r($employe_result);
                                                       // echo $employe_result[0]->user_id;
                                                    if (isset($employe_result) && !empty($employe_result)) { 
                                                        $count = 0;
                                                    foreach($employe_result as  $val ){
                                                        $count++;
                                                        ?>
                                                            <tr>
                                                                <td class="<?php if($val->status==0) { echo "danger"; } ?>"><?php echo $count; ?></td>
                                                                <td class="<?php if($val->status==0) { echo "danger"; } ?>"><?php echo $val->user_id; ?></td>
                                                                <td class="<?php if($val->status==0) { echo "danger"; } ?>"><?php echo $val->user_name; ?></td>
                                                                <td class="<?php if($val->status==0) { echo "danger"; } ?>"><?php echo $val->branch_name; ?></td>
                                                                <td class="<?php if($val->status==0) { echo "danger"; } ?>"><?php echo $val->role_type; ?></td>

                                                                <td class="<?php if($val->status==0) { echo "danger"; } ?> text-center"> 
                                                                                             
                                                                <a href="<?php echo base_url('index.php/Masters/edit_employe/'.$val->id)?>" class="btn btn-primary btn-xs" ><i class="fa fa-pencil-square-o"></i></a> &nbsp;&nbsp;
                                                    ...
                                                    <a  onclick="get_delete_pop('<?php echo $val->id;?>','ckb_users');" href="#" class="btn btn-danger btn-xs "><i class="fa fa-close"></i> </a>
        ...
        $(document).ready(function () {
			...
			$(document).ready(function() {
			    $(document).on('click','.inactive_doc',function(){
					var resp = confirm("Do you want to Inactive this Employee?");
					if (resp == true) {
						var inactive_id = $(this).attr('inactive_id');
						//alert(inactive_id);
						$.ajax({
							type : "POST", 
							url : "<?php echo base_url(); ?>index.php/Masters/inactive_employe",
			...
			$(document).ready(function() {
			    $(document).on('click','.active_doc',function(){
					var resp = confirm("Do you want to active this Employee?");
					if (resp == true) {
						var active_id = $(this).attr('active_id');
						//alert(active_id);
						$.ajax({
							type : "POST", 
							url : "<?php echo base_url(); ?>index.php/Masters/active_employe",
  ...
  swal({   
      ...
      text: "You will not be able to recover this Employee and all the data related this employee also affected!!!",   
      type: "warning",   
