...
<html>
    <head>
        ...
        <title><?php echo $this->session->userdata('client_name'); ?> | Add Lab Diagnostics list</title>
                                    ...
                                    <div class="modal-dialog"> 
                                        <div class="modal-content"> 
                                            <div class="modal-header"> 
                                                ...
                                                <h4 class="modal-title">ADD Lab Diagnostics </h4> 
                                            </div> 
                                            <form name="healthlab_form" id="healthlab_form" class="form-horizontal form-bordered" action="<?php echo base_url('index.php/Healthcare/add_lab_diag'); ?>" enctype="multipart/form-data"  data-parsley-validate method="post" autocomplete="off">
                                            <div class="modal-body"> 
                                                <div class="row"> 
                                                    <div class="col-md-6"> 
                                                        <div class="form-group"> 
                                                            <label for="Aviary Name" class="control-label">Lab Diagnostics</label>
                                                            <!-- <input type="hidden" name="auto_id" id="auto_id" class="form-control" value="<?php //$last_id=$last_id['id']+1; echo "LB00".$last_id; ?>">  -->
                                                            <input type="hidden" name="created_by" id="created_by" class="form-control" value="<?php echo $this->session->userdata('user_id') ?>"> 
                                                            <input type="text" name="diag_name" id="diag_name" class="form-control" required>
                                                        </div> 
                                                    </div> 
                            ...
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h3 class="panel-title">Lab Diagnostics</h3>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-sm-12 col-xs-12">
                                                <table id="datatable" class="table table-striped table-bordered">
                                                    ...
                                                    <tbody>
                                                    <?php foreach($diag as $k => $val ){ ?>
                                                            <tr>
                                                                <td class="<?php if($val['status']==0) { echo "danger"; } ?>"><?php echo $k+1; ?></td>
                                                                <td class="<?php if($val['status']==0) { echo "danger"; } ?>"><?php echo $val['diag_name']; ?></td>
                                                                <td class="<?php if($val['status']==0) { echo "danger"; } ?> text-center"> 
                                                                                             
                                                                <a href="<?php echo base_url('index.php/Healthcare/edit_healthcare_lab/'.$val['id']); ?>" class="btn btn-primary btn-xs" ><i class="fa fa-pencil-square-o"></i></a> &nbsp;&nbsp;
        ...
        $(document).ready(function () {
			...
			$(document).ready(function() {
			    $(document).on('click','.inactive_doc',function(){
					var resp = confirm("Do you want to Inactive this lab Diagnostics?");
					if (resp == true) {
						var inactive_id = $(this).attr('inactive_id');
						//alert(inactive_id);
						$.ajax({
							type : "POST", 
							url : "<?php echo base_url(); ?>index.php/Healthcare/inactive_lab",
			...
			$(document).ready(function() {
			    $(document).on('click','.active_doc',function(){
					var resp = confirm("Do you want to active this lab Diagnostics?");
					if (resp == true) {
						var active_id = $(this).attr('active_id');
						//alert(active_id);
						$.ajax({
							type : "POST", 
							url : "<?php echo base_url(); ?>index.php/Healthcare/active_lab",
