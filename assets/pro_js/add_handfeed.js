  ...
  //get_brooder_details('B004')
	...
	$("#actn_feed").keyup(function(){  //showing feed volume
	   ...
	   for (let index = 1; index <=get_feed; index ++) {
		...
		feed_html +='<input type="text"name="feed'+index+'" id="feed'+index+'" onkeyup="calc_total();" placeholder="Feed'+index+'" class="SmallInput form-control feed_add"  value=" " required/>';
		feed_html +=' </div>';
		feed_html +=' </div>';
	}
   ...
   achieved();
	...
	// $(".feed_add").on("change input keyup ", function(){
	// 	alert("test");
	// 	calc_total();
	// });
...
function calc_total(){
	...
	$( ".feed_add" ).each(function() {
		//alert("test");
    // console.log("row amnt - "+$(this).val());

    headcount_sum += parseFloat(Number($(this).val()));
	//console.log(headcount_sum);
  
    });
    $('#actualFeed_vday').val(headcount_sum);
	
}
// function get_act_feed(index)
// {
// 		var feed_val=$('#feed'+index).val();
// 		var prev_index = index-1;

// 	if($('#feed'+prev_index).length){
// 	//	var total_val =[];
// 	for(i=1; i<index; i++){
// 		//alert(index-i);
// 		total_val= (parseInt($('#feed'+index).val()) + parseInt($('#feed'+(index-i)).val()));
// 		total_val += total_val;
// 	}
// 	$(actualFeed_vday).val(total_val);
// 	}else{
// 		$(actualFeed_vday).val(feed_val);
// 	}
...
//get_weightgain(age,getid);

}

function get_weightgain(getid){
	$.ajax({
		url: BASE_URL + 'Handfeeding/get_handfeed_weight',
		method: "POST",
		data: {
...
$('#handfeedForm').submit(function(e) {
    var formData = new FormData(this);
    e.preventDefault();
       // alert(formData );
    $.ajax({  
        url:BASE_URL + 'Handfeeding/addHandfeed', 
        method:"POST",  
        data: formData,
        cache:false,
        contentType: false,
