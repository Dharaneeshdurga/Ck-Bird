...
function get_preweaning_list(){
    //$('#hide_pdate').hide();
    var ct = $('#preweaning_tb').DataTable({
        ...
        'ajax': {
            'url': BASE_URL + 'Preweaning/get_preweaning_list',
            'data': function(data) {
    ...
    $('#preweaning_tb tbody').on('click', 'td.details-control', function() {
...
function get_move_Weaning(incub_row_id){
...
$('#weaning_form').submit(function(e) {
   // var p_date = $('#pre_weaning_date').val();
    ...
    e.preventDefault();
    $.ajax({  
        url:BASE_URL + 'Preweaning/move_weaning', 
        method:"POST",  
        data: formData,
        cache:false,
        contentType: false,
 ...
 $('#health_form').submit(function(e) {
    // var p_date = $('#pre_weaning_date').val();
     ...
     $.ajax({  
         url:BASE_URL + 'Preweaning/change_health_status', 
         method:"POST",  
         data: formData,
         cache:false,
         contentType: false,
 ...
 function get_move_sale(incub_row_id){
    //var data = incub_row_id;
    $.ajax({
     url: BASE_URL + 'Preweaning/preweaning_move_sale',
     method: "POST",
     data:{
     ...
     success: function(data) {
         var get_html = '<input type="hidden" name="p_id" class="SmallInput"  value="'+data.full_detail[0].auto_id+'">';
          ...
          get_html += '<input type="hidden" name="proven" class="SmallInput"  value="Preweaning">';
 ...
 $('#sale_form').submit(function(e) {
    // var p_date = $('#pre_weaning_date').val();
     ...
     e.preventDefault();
     $.ajax({  
         url:BASE_URL + 'Preweaning/move_sale', 
         method:"POST",  
         data: formData,
         cache:false,
         contentType: false,
 ...
 function revert_to_handfeed(egg_no){
    var resp = confirm("Are you sure want to revert this?");
    if (resp == true) {

    $.ajax({
     url: BASE_URL + 'Preweaning/revert_to_handfeed',
     method: "POST",
     data:{
         "egg_no":egg_no,
     ...
     success: function(data) {
        if(data.logstatus =='success'){
            ...
            $.Notification.autoHideNotify(
                'success', 
                'top right', 
                'Successfully Reverted to Handfeeding ..!',
                ''
            );		
            setTimeout(
                function() {
