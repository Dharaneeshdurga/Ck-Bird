...
function get_handfeedhistory_list(){
  ...
  // var data = incub_row_id;
    var ct = $('#get_handfeed_update').DataTable({
        ...
        'ajax': {
            'url': BASE_URL + 'Handfeeding/get_handfeed_update',
            'data': function(data) {
                data.handfeed_id = incub_row_id;
        ...
        createdRow: function( row, data, dataIndex ) {
            if(data.status > 0){
                ...
                if(data.mail_status != "mailed"){
                    var message_body = "From handfeed Details";
    ...
    $('#get_handfeed_update tbody').on('click', 'td.details-control', function() {
...
function feed_history_pop(incub_row_id,date){
  ...
  // $("#weightloss_id").html(get_html);
   $('#feed-modal').modal('show');
             ...
             $.ajax({
                 url: BASE_URL + 'Handfeeding/get_handfeed_feed',
                 method: "POST",
                 data: {
